...
//         >
//           <button className="absolute top-5 right-4 text-gray-400" onClick={() => setShowForm(false)}>
//             <RiCloseLargeFill className="w-6 h-6" />
//           </button>
//           <h2 className="text-xl font-semibold mb-4">What's happening?</h2>
//           <input className="mb-4 w-full border p-2 rounded-lg" type="file" onChange={handleFileChange} accept="image/*" />
//           <input className="mb-4 w-full border p-2 rounded-lg" type="text" placeholder="What's up?" value={text} onChange={handleTextChange} />
//           <input className="mb-4 w-full border p-2 rounded-lg" type="text" placeholder="Enter Spotify Link" value={spotifyUrl} onChange={handleSpotifyUrlChange} />
...
//             {post.image && <img src={post.image} alt="User Upload" className="w-full h-auto rounded-lg mb-3" />}
//             <p className="text-base text-white">{post.text}</p>
//             {post.spotifyUrl && <iframe src={`https://open.spotify.com/embed/track/${post.spotifyUrl.split('/').pop()}`} className="w-full h-24 md:h-32 lg:h-40 rounded-lg" allow="encrypted-media"></iframe>}
