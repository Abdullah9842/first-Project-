...
const SpotifyInput: React.FC<SpotifyInputProps> = ({ spotifyUrl, setSpotifyUrl }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // التحقق من صحة الرابط باستخدام تعبير منتظم لسبوتيفاي
    const regex = /^(https?:\/\/)?(www\.)?(spotify\.com\/)(.*)$/;
    ...
    <div className="relative">
      ...
      {!showInput ? (
        ...
        <input
          ...
          onBlur={() => setShowInput(false)}
          className="px-2 py-2 bg-white rounded-2xl text-gray-500 focus:outline-none focus:ring-2 transition w-full mt-6" // إضفاء مساحة أعلى لتجنب التداخل
          autoFocus
        />
      )}
